...
def get_retained_samples_for_product(
    ...
    if id:
        retained_samples = (
            db.query(models.SampleRetained, models.Product)
            .join(models.Product)
            .filter(models.SampleRetained.id == id)
            .one()
        )
        if not retained_samples:
            raise HTTPException(status_code=404, detail="Retained samples not found")

    else:
        retained_samples = (
            db.query(models.SampleRetained).offset(skip).limit(limit).all()
        )
...
def update_retained_sample(
    id: str, sample: schemas.SampleRetained, db: Session = Depends(get_db)
):
    """
    ...
    existing_sample = (
        db.query(models.SampleRetained).filter(models.SampleRetained.id == id).first()
    )
