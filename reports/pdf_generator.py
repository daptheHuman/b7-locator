...
class PDF(FPDF):
    def __init__(self, orientation: Literal['', 'portrait', 'p', 'P', 'landscape', 'l', 'L'] = "portrait", unit: float | Literal['pt', 'mm', 'cm', 'in'] = "mm", format: tuple[float, float] | Literal['', 'a3', 'A3', 'a4', 'A4', 'a5', 'A5', 'letter', 'Letter', 'legal', 'Legal'] = "A4", font_cache_dir: Literal['DEPRECATED'] = "DEPRECATED") -> None:
    ...
    def footer(self) -> None:
        ...
        if (self.finished):
            ...
            self.image(APPROVED_STAMP, x=127, y=175, w=25)
...
def generate_destroy_report(samples: List[schemas.SampleProductJoin], date: date, page: int = 1):
    ...
    with pdf.table(line_height=20, col_widths=COLUMN_WIDTHS, text_align=Align.C) as table:
        headers = table.row()
        headers.cell(
            img=B7_LOGO,  colspan=2,  padding=3)
        headers.cell(
            "Form Pemusnahan Retained Sampel Finished Goods", colspan=7, style=FontFace(size_pt=16))
